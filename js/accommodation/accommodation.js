...
function renderHotels() {
  const data = allHotelData[currentRegion] || [];
  ...
  pageData.forEach((hotel, index) => {
    ...
    card.innerHTML = `
      <div class="hotel-card-image">
        ...
        <div class="hotel-badge">${regionNames[hotel.region]}</div>
      </div>
...
hotelList.addEventListener("click", (e) => {
  ...
  if (bookBtn) {
    ...
    const data = allHotelData[region] || [];
    const selectedHotel = data.find(h => h.id === hotelId);

    if (selectedHotel) {
      // 예약 페이지로 이동 또는 모달 열기
      window.location.href = `../hotel/hotel-booking.html?hotelId=${hotelId}&region=${region}`;
    }
    return;
  }
...
if (regionParam && allHotelData[regionParam]) {
